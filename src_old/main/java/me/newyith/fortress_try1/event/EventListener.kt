package me.newyith.fortress_try1.event
...
class EventListener(plugin: JavaPlugin) : Listener {
	...
	fun onChunkLoad(event: ChunkLoadEvent) {
		GeneratorRuneManager.forWorld(event.world).onChunkLoad(event.chunk)
	}

	@EventHandler(ignoreCancelled = true)
	fun onChunkUnload(event: ChunkUnloadEvent) {
		GeneratorRuneManager.forWorld(event.world).onChunkUnload(event.chunk)
	}
}