...
/*
	...
	private RuneState state = RuneState.NULL;
	...
	public boolean isRunning() {
		return this.state == RuneState.RUNNING;
	}

	private boolean isPaused() {
		return this.state == RuneState.PAUSED;
	}
	...
	private void updateState() {
		...
		if (fuelTicksRemaining > 0) {
			if (this.isPowered()) {
				this.setState(RuneState.PAUSED);
			} else {
				this.setState(RuneState.RUNNING);
			}
		} else {
			this.setState(RuneState.NEEDS_FUEL);
		}
	}

	private void setState(RuneState state) {
		if (this.state != state) {
			switch (state) {
				case RUNNING:
	...
	private void updateFuelRemainingDisplay(long ms) {
		int glowstoneDustInChest = this.countGlowstoneDustInChest();
