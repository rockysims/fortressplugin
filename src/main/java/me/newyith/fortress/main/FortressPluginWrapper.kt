...
//	IDEALLY: ideally by not storing things at global level (reconstruct instead): we allow world data folders to be added/removed without breaking things
...
//IDEA: +1 layer converted per animation tick for each nether quartz (item) in fuel chest

//SERIOUSLY CONSIDER: maybe generator runs on quartz (item) instead of glowstone dust
//	quartz is much easier to gather and can be relaxing to mine
//	glowstone could fuel disruptor (1 glowstone = 8 blocks anti protect for 60 minutes)
//		or 1 glowstone = 1 shot but it applies anti protect to small sphere around impact for 60 minutes
//ANSWER: stick with glowstone because it really isn't much harder to get and it comes in big pockets so it feels more rewarding
//	nether quartz (item) could be fuel for disruptor

//TODO: make each glowstone dust last 4 hours instead of 1 hour (works out to 10 days per 60 dust)

//player has to stand on disruptor and right click with blaze rod to fire
//	provides way for defender team to temporarily stop disruptor from firing (kill gunner)
//	provides way for attacker team to use ammunition carefully
//		put blaze rod in chest to auto fire
//PROBLEM?: hard to explain to players that you literally stand on the disruptor and right click
...
//		on enabled, sphere contracts slowly
...
//TODO: maybe play sound when generator is created, enabled, enabled
...
//TODO: allow wall particles to be enabled via config
//TODO: allow debug messages to be enabled via config (maybe default to enabled but only if releaseBuild)
