...
//TODO: try saving onWorldSave then see if I can get it to leave behind any abandoned bedrock by killing server
//Reliable Saving Solution (if just saving onWorldSave doesn't work):
//	onGenerate, add wall points to potentialBedrock.json (Map<World, Map<Point, Material>>)
//	onSave, clear potentialBedrock.json (then add back runes currently animating)
//	onLoad, ensure all actual bedrock in potentialBedrock is generated by a rune that still exists
//	make periodic save be every 5 minutes or less so potentialBedrock wont get too big
//	also save when a world saves (if world save hasn't trigger a save for 10 seconds)?

//Vehicle Glitch Solution:
//	cancel pearls thrown while player is vehicle and in generated point(s)
