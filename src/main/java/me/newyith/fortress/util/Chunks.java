...
public class Chunks {
	...
	// 		chunk.load(); //not sure if this line is needed when followed by chunk.setForceLoaded(true)
	// 		chunk.setForceLoaded(true);
	// 	}
	// }

	// //NOTE: use of setForceLoaded() in loadAndPreventUnload and allowUnload is untested

	// public static void allowUnload(World world, ChunkBatch batch) {
	// 	batchesForWorld(world).remove(batch);
	// 	keepLoadedChunkXZsByWorld.remove(world.getName()); //ensure recompute on next access

	// 	//stop force loading chunnks in batch (unless still force loaded by some other batch)
	// 	Set<ChunkXZ> forceLoadedChunks = chunkXZsForWorld(world);
	// 	for (Chunk chunk : batch.getChunks(world)) {
	// 		if (!forceLoadedChunks.contains(new ChunkXZ(chunk))) {
